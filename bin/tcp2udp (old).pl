#!/usr/bin/perl
# Christian Fenzl, christiantf@gmx.at 2018
# This script is a gateway from Loxone to any TCP devices. 
# As Loxone does not support bidirectional communication, the TCP answers of devices are transmitted as UDP to Loxone.
...
my $cfgfilename = "$lbpconfigdir/tcp2udp.cfg";
# tolog("INFORMATION", "Reading Plugin config $cfg");
if (! (-e $cfgfilename)) {
	print STDERR "TCP2UDP configuration does not exist. Terminating.\n";
	unlink $pidfile;
	exit(0);
}
...
my $activated = $cfg->param("TCP2UDP.activated");
...
if(! is_true($activated) && ! $option_activate) {	
	print STDERR "TCP2UDP Plugin is not activated in config file. That's ok. Terminating.\n";
	unlink $pidfile;
	exit(0);
}
